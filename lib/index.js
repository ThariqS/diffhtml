...
Element.prototype.setDiffHTML = function(newHTML){
  ...
  for (var key in patches) {
    var p = patches[key];
    if (p && p.vNode && p.vNode.properties && p.vNode.properties.className == 'math'){
      edited_nodes.push({ id: p.vNode.properties.id });
    }
  }
...
Object.defineProperty(Element.prototype, 'diffHTML', {
  ...
  set: function(newHTML) {
    ...
    for (var key in patches) {
      var p = patches[key];
      if (p && p.vNode && p.vNode.properties && p.vNode.properties.className == 'math'){
        console.log("EDITING A MATH OBJECT");
      }
    }
